@php
    $urlPihak1 = route('bast.pemakaian.sign', [$record, 'pihak1']);
    $urlPihak2 = route('bast.pemakaian.sign', [$record, 'pihak2']);
    
    // QR Code untuk Pihak 2 (Pegawai) agar bisa langsung scan dari smartphone
    $qrSvg = \SimpleSoftwareIO\QrCode\Facades\QrCode::format('svg')->size(150)->margin(1)->generate($urlPihak2);
...
<div class="space-y-5 text-slate-800">
    ...
    <div class="p-3.5 bg-slate-50 rounded-xl border border-slate-200 text-xs space-y-1.5">
        <div class="flex justify-between items-center">
            ...
            <span class="font-bold text-slate-900 font-mono">{{ $record->nomor_bast ?? 'DRAFT (Belum Terbit)' }}</span>
        </div>
        <div class="flex justify-between items-center">
            <span class="text-slate-500 font-medium">Penerima (Pihak 2):</span>
            <span class="font-semibold text-slate-800">{{ $record->pihak_kedua_display }}</span>
        </div>
        <div class="flex justify-between items-center">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <div class="p-3.5 rounded-xl border {{ $record->ttd_pihak1_url ? 'border-emerald-200 bg-emerald-50/50' : 'border-slate-200 bg-white' }} space-y-2.5">
            <div class="flex items-center justify-between">
                <span class="text-xs font-bold text-slate-700">Pihak 1 (Pejabat)</span>
            ...
            <div class="text-xs text-slate-600">
                <div class="font-medium text-slate-900 truncate">{{ $record->pihakPertama->nama ?? 'Pejabat BMN' }}</div>
                <div class="text-[11px] text-slate-500">NIP. {{ $record->pihak_pertama_nip ?? '-' }}</div>
            </div>
        ...
        <div class="p-3.5 rounded-xl border {{ $record->ttd_pihak2_url ? 'border-emerald-200 bg-emerald-50/50' : 'border-slate-200 bg-white' }} space-y-2.5">
            <div class="flex items-center justify-between">
                <span class="text-xs font-bold text-slate-700">Pihak 2 (Penerima)</span>
            ...
            <div class="text-xs text-slate-600">
                <div class="font-medium text-slate-900 truncate">{{ $record->pihak_kedua_display }}</div>
                <div class="text-[11px] text-slate-500">Pegawai Pemakai</div>
            </div>
    ...
    <div class="p-4 bg-gradient-to-br from-slate-50 to-red-50/30 rounded-xl border border-red-100 flex flex-col sm:flex-row items-center gap-4 text-center sm:text-left">
        ...
        <div class="space-y-1">
            ...
            <p class="text-[11px] text-slate-600 leading-relaxed">
                Minta pegawai/staf penerima mengarahkan kamera HP ke QR Code ini untuk menandatangani BAST langsung menggunakan jari di smartphone mereka tanpa perlu login ke dashboard admin.
            </p>
            <div class="pt-1">
                <a href="{{ $urlPihak2 }}" target="_blank" class="text-[11px] font-semibold text-red-700 hover:underline">
