...
<head>
    ...
    <title>Verifikasi Dokumen BAST Pengembalian - BAWASLU</title>
    ...
    <main class="max-w-3xl mx-auto px-4 -mt-4 space-y-5">
        ...
        <div class="bg-white rounded-2xl shadow-md border border-slate-200/80 p-6 text-center">
            @if($isFinal)
                ...
                <span class="inline-block px-3 py-1 bg-emerald-100 text-emerald-800 text-xs font-extrabold uppercase tracking-wider rounded-full mb-1">
                    ✓ Dokumen Pengembalian Sah & Terverifikasi
                </span>
                <h2 class="text-xl font-extrabold text-slate-900 mt-1">Berita Acara Pengembalian BMN</h2>
                <p class="text-xs text-slate-500 max-w-md mx-auto mt-1">
                    Dokumen pengembalian aset ini terdaftar resmi dan telah ditandatangani secara digital dalam Sistem Informasi Manajemen Aset Bawaslu.
                </p>
            @else
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200/80 p-5 space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                <div>
                    <span class="text-xs text-slate-500 block">Nomor BAST Pengembalian:</span>
                    <span class="font-mono font-bold text-slate-900 text-base">{{ $header->nomor_bast_pengembalian ?? 'DRAFT' }}</span>
                </div>
                <div>
                    <span class="text-xs text-slate-500 block">Jenis Dokumen:</span>
                    <span class="font-semibold text-slate-800">BAST Pengembalian BMN</span>
                </div>
                <div>
                    <span class="text-xs text-slate-500 block">Tanggal Pengembalian:</span>
                    <span class="font-medium text-slate-800">{{ $header->tanggal ? $header->tanggal->translatedFormat('d F Y') : '-' }}</span>
                </div>
                <div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <!-- Pihak Menyerahkan (Pegawai) -->
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200/80 p-5 space-y-3">
                <div class="flex items-center justify-between">
                    <span class="text-xs font-bold uppercase tracking-wider text-red-700">Yang Menyerahkan</span>
                    @if($isSignedPihak1)
                ...
                <div class="text-xs text-slate-500">Pihak Pertama (Pegawai yang Mengembalikan)</div>
                <div>
                    <h4 class="font-bold text-slate-900 text-sm">{{ $header->pihak_menyerahkan_display }}</h4>
                    @if($header->pihak_menyerahkan_tipe === 'internal' && $header->pihakMenyerahkan)
                        <p class="text-xs text-slate-600 mt-0.5">NIP. {{ $header->pihak_menyerahkan_nip }}</p>
                        <p class="text-xs text-slate-500">{{ $header->pihakMenyerahkan->jabatan ?? 'Pegawai' }} - {{ $header->pihakMenyerahkan->unit_kerja ?? '-' }}</p>
                    @endif
                </div>
            </div>

            <!-- Pihak Menerima (Pejabat BMN) -->
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200/80 p-5 space-y-3">
                <div class="flex items-center justify-between">
                    <span class="text-xs font-bold uppercase tracking-wider text-red-700">Yang Menerima</span>
                    @if($isSignedPihak2)
                        <span class="text-[11px] font-bold bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full">✓ TTD Sah</span>
                    @else
                        <span class="text-[11px] font-semibold bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full">Belum TTD</span>
                ...
                <div class="text-xs text-slate-500">Pihak Kedua (Pejabat / Pengelola BMN)</div>
                <div>
                    <h4 class="font-bold text-slate-900 text-sm">{{ $header->pihakMenerima->nama ?? '-' }}</h4>
                    <p class="text-xs text-slate-600 mt-0.5">NIP. {{ $header->pihak_menerima_nip ?? '-' }}</p>
                    <p class="text-xs text-slate-500">{{ $header->pihakMenerima->jabatan ?? 'Pejabat Penerima' }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200/80 p-5 space-y-3">
            <h3 class="text-sm font-bold text-slate-900 flex items-center justify-between">
                <span>Rincian Barang yang Dikembalikan</span>
                ...
                <table class="w-full text-left text-xs border-collapse">
                    <thead>
                        <tr class="border-b border-slate-200 bg-slate-50 text-slate-600 font-semibold">
                            ...
                            <th class="py-2.5 px-3">Kondisi Kembali</th>
                        </tr>
                    </thead>
                            ...
                            <tr>
                                ...
                                <td class="py-2.5 px-3">
                                    <span class="px-2 py-0.5 rounded-md text-[11px] font-semibold bg-emerald-50 text-emerald-700 border border-emerald-200">
                                        {{ $itemDetail->kondisi_saat_kembali ?? ($itemDetail->item->kondisi ?? 'Baik') }}
                                    </span>
                                </td>
                            </tr>
                        @empty
